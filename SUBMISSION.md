...
- **LLM (локальный):** IR-нарратив / отчёт генерит **локальная модель** — Ollama, `qwen2.5:7b` on-prem на `127.0.0.1:11434` (env-override `OLLAMA_URL`/`OLLAMA_MODEL`), **без ключа и без внешнего/облачного API**. LLM **никогда** не считает score — пишет только текст отчёта. При недоступном Ollama — детерминированный mock-fallback (offline). **Ноль внешних API во всём продукте: каждая модель локальна.**
