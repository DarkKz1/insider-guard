...
- **Независимая AI-модель (крит. #2):** ДВЕ собственные **unsupervised anomaly-модели на устройстве** — robust modified z-score (median/MAD, `server/lib/fmt.js` → `madZScores`, `server/engine.js` → `peerGroupMad`) **и полноценный Isolation Forest** (Liu/Ting/Zhou, ICDM 2008; чистый JS, **0 ML-зависимостей**, обучается на всём корпусе, виден в UI — чип `iForest` + MODEL CARD; seeded-детерминизм mulberry32/seed 1337, `index.html` → `buildIForest`). Без внешних сервисов. Поверх вероятностной AI — детерминированный **0-FP honeytoken-слой** (defense-in-depth): honeytoken поведенчески не аномален, поэтому IF даёт по нему низкий score — и это **по замыслу**, детерминированная ловушка ловит ровно то, что вероятностная модель не видит.
