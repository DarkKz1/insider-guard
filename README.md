...
- **Две независимые AI-модели на устройстве (крит. #2)** — (1) **robust modified z-score (median/MAD)**, сигнал «×N от своей нормы И от пиров» (`server/lib/fmt.js` → `madZScores`, `server/engine.js` → `peerGroupMad`); (2) **полноценный Isolation Forest** (Liu/Ting/Zhou, ICDM 2008) — реализован с нуля на чистом JS, **0 ML-зависимостей**, обучается на всём корпусе, **виден в UI** (чип `iForest` + развёрнутый MODEL CARD), seeded-детерминизм (mulberry32, seed 1337; `index.html` → `buildIForest`). Обе — без внешних сервисов, на устройстве. Поверх вероятностной AI — детерминированный **0-FP honeytoken-слой** (defense-in-depth): honeytoken поведенчески не аномален → IF даёт низкий score, и это **по замыслу**, ловушка покрывает ровно то, что вероятностная модель не видит. Внешний `claude-opus-4-8` — только опциональный черновик отчёта (offline-fallback), score не считает.
...
- **Скоринг-движок** (Σ===score, источник вердикта) — **детерминированные UEBA-эвристики +
  ...
  2008, чистый JS, обучается на корпусе, виден в UI) как отдельные anomaly-сигналы, НЕ входящие в Σ
  score. То есть IF — **реализован и работает**, а не «заготовка за флагом».
